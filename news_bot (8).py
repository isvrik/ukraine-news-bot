"""
Бот для отслеживания новостей по теме "переговоры по Украине"
и отправки ссылок в Telegram-канал.

Как это работает:
1. Забирает новости из Google News RSS по заданным ключевым словам
   (Google News сам агрегирует ТАСС, РИА, Reuters, AP, Axios и тысячи
   других источников — не нужно искать RSS каждого сайта отдельно).
2. Дополнительно проверяет пару прямых RSS-лент (ТАСС, РИА) для надёжности.
3. Отбирает только те новости, где есть нужные ключевые слова.
4. Отсеивает платные/подписочные источники, устаревшие новости и
   новости с "будущей" датой публикации (баг ленты).
5. Не отправляет повторно то, что уже было отправлено (хранит список
   отправленных ссылок в файле seen_links.json).
6. Из всех подходящих новостей за один запуск отправляет только 1-2
   самых актуальных, отдавая приоритет "инсайдерским" материалам
   (со ссылкой на источники, эксклюзивам и т.п.).
7. Если ни одна новость не прошла фильтры — ничего не отправляет и
   просто ждёт следующего запуска.
"""

import calendar
import json
import os
import re
import time
from difflib import SequenceMatcher
from pathlib import Path
from urllib.parse import quote

import feedparser
import requests
from deep_translator import GoogleTranslator

# ========== НАСТРОЙКИ ==========

# Токен бота и канал берутся из переменных окружения (секретов GitHub),
# а не хранятся в коде напрямую — это безопаснее.
TELEGRAM_TOKEN = os.environ["TELEGRAM_TOKEN"]
CHANNEL_ID = os.environ.get("CHANNEL_ID", "@news_isv_bot")

# Ключевые слова, по которым фильтруются новости (для лент без
# собственного поискового запроса, например прямых RSS ТАСС/РИА).
KEYWORDS = [
    "переговоры по украине",
    "мирный план украина",
    "мирное соглашение украина",
    "прекращение огня украина",
    "перемирие украина",
    "зеленский трамп переговоры",
    "путин переговоры украина",
    "ukraine peace talks",
    "ukraine negotiations",
]

# Поисковый запрос для Google News на русском (агрегирует много источников сразу).
# Требуется слово "Украина" ВМЕСТЕ с конкретной темой переговоров/мира,
# а не любое упоминание слова "переговоры" рядом с Украиной.
GOOGLE_NEWS_QUERY_RU = (
    'Украина ("мирные переговоры" OR "переговоры по Украине" OR '
    '"прекращение огня" OR "мирный план" OR "мирное соглашение" OR '
    "Уиткофф OR Кушнер)"
)
GOOGLE_NEWS_RSS_RU = (
    f"https://news.google.com/rss/search?q={quote(GOOGLE_NEWS_QUERY_RU)}"
    "&hl=ru&gl=RU&ceid=RU:ru"
)

# Поисковый запрос для Google News на английском.
# Сужено строго до темы ПЕРЕГОВОРОВ (а не любой войны/ceasefire/сделки
# в целом) и до источников, которые обычно первыми сообщают о
# дипломатических новостях (Reuters, AP, Axios) — остальные СМИ обычно
# просто перепечатывают их материалы с задержкой, создавая дубли и шум.
GOOGLE_NEWS_QUERY_EN = (
    'Ukraine ("peace talks" OR "peace negotiations" OR negotiations OR '
    "Witkoff OR Kushner) (site:reuters.com OR site:apnews.com OR site:axios.com)"
)
GOOGLE_NEWS_RSS_EN = (
    f"https://news.google.com/rss/search?q={quote(GOOGLE_NEWS_QUERY_EN)}"
    "&hl=en&gl=US&ceid=US:en"
)

# Ленты, которые уже отфильтрованы самим поисковым запросом Google News —
# для них не нужна повторная проверка по ключевым словам.
PRE_FILTERED_FEEDS = {
    GOOGLE_NEWS_RSS_RU,
    GOOGLE_NEWS_RSS_EN,
}

# Дополнительные прямые RSS-ленты (можно добавлять свои)
DIRECT_FEEDS = [
    "https://tass.ru/rss/v2.xml",
    "https://ria.ru/export/rss2/archive/index.xml",
]

ALL_FEEDS = [
    GOOGLE_NEWS_RSS_RU,
    GOOGLE_NEWS_RSS_EN,
] + DIRECT_FEEDS

SEEN_FILE = Path(__file__).parent / "seen_links.json"

# Сколько последних отправленных заголовков хранить для проверки на похожесть
RECENT_TITLES_LIMIT = 150

# Порог похожести заголовков (0.0-1.0) для дедупликации
DUPLICATE_SIMILARITY_THRESHOLD = 0.6

# Сколько новостей максимум отправлять за один запуск бота
MAX_NEWS_PER_RUN = 2

# Новость считается устаревшей, если она старше этого числа часов
MAX_NEWS_AGE_HOURS = 6

# Допуск на "будущую" дату публикации (защита от небольшого рассинхрона
# часов между сервером ленты и раннером GitHub Actions)
FUTURE_TOLERANCE_MINUTES = 10

# Домены с платным доступом / обязательной подпиской — такие новости
# не публикуем, т.к. читатель не сможет открыть статью бесплатно.
PAYWALLED_DOMAINS = [
    "bloomberg.com",
    "wsj.com",
    "nytimes.com",
    "ft.com",
    "economist.com",
    "washingtonpost.com",
    "telegraph.co.uk",
    "thetimes.co.uk",
    "barrons.com",
]

# Фразы-маркеры платного доступа, которые иногда попадают в анонс новости
PAYWALL_MARKERS = [
    "subscribe to read",
    "subscribe to continue",
    "for subscribers",
    "subscriber-only",
    "requires a subscription",
    "paywall",
    "только для подписчиков",
    "по подписке",
]

# Маркеры "инсайдерской" новости — такие материалы получают приоритет
# при выборе, какие 1-2 новости отправить за прогон.
INSIDER_KEYWORDS = [
    "источник",
    "источники",
    "стало известно",
    "как стало известно",
    "инсайд",
    "по данным источников",
    "источник, знакомый",
    "exclusive",
    "sources say",
    "sources familiar",
    "according to sources",
    "people familiar with",
    "obtained by",
    "leaked",
    "insider",
]

# ========== ЛОГИКА ==========


def load_seen() -> dict:
    """
    Возвращает {"links": [...], "titles": [...]}.
    Поддерживает старый формат файла (просто список ссылок) для плавного
    перехода на новую структуру.
    """
    if SEEN_FILE.exists():
        try:
            data = json.loads(SEEN_FILE.read_text(encoding="utf-8"))
            if isinstance(data, list):
                return {"links": data, "titles": []}
            return {
                "links": data.get("links", []),
                "titles": data.get("titles", []),
            }
        except Exception:
            return {"links": [], "titles": []}
    return {"links": [], "titles": []}


def save_seen(seen_links: set, recent_titles: list) -> None:
    # Храним не более 2000 последних ссылок и последние RECENT_TITLES_LIMIT
    # нормализованных заголовков, чтобы файл не рос бесконечно
    data = {
        "links": list(seen_links)[-2000:],
        "titles": recent_titles[-RECENT_TITLES_LIMIT:],
    }
    SEEN_FILE.write_text(
        json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8"
    )


def normalize_title(title: str) -> str:
    """
    Приводит заголовок к простому виду для сравнения на похожесть:
    убирает приписку источника в конце (Google News добавляет " - Название СМИ"),
    убирает пунктуацию и лишние пробелы, переводит в нижний регистр.
    """
    core = re.split(r"\s+-\s+[^-]+$", title)[0]
    core = re.sub(r"[^\w\s]", "", core, flags=re.UNICODE).lower()
    core = re.sub(r"\s+", " ", core).strip()
    return core


def is_duplicate_title(title: str, recent_titles: list) -> bool:
    normalized = normalize_title(title)
    if not normalized:
        return False
    for prev in recent_titles:
        if SequenceMatcher(None, normalized, prev).ratio() >= DUPLICATE_SIMILARITY_THRESHOLD:
            return True
    return False


def is_relevant(title: str, summary: str) -> bool:
    text = f"{title} {summary}".lower()
    return any(kw.lower() in text for kw in KEYWORDS)


def is_paywalled(title: str, summary: str, link: str) -> bool:
    link_lower = (link or "").lower()
    if any(domain in link_lower for domain in PAYWALLED_DOMAINS):
        return True
    text = f"{title} {summary}".lower()
    return any(marker in text for marker in PAYWALL_MARKERS)


def insider_score(title: str, summary: str) -> int:
    text = f"{title} {summary}".lower()
    return 1 if any(kw in text for kw in INSIDER_KEYWORDS) else 0


def get_published_ts(entry) -> float | None:
    """Возвращает время публикации записи как unix-timestamp (UTC), либо None."""
    parsed = entry.get("published_parsed") or entry.get("updated_parsed")
    if not parsed:
        return None
    try:
        return calendar.timegm(parsed)
    except Exception:
        return None


def is_fresh(entry) -> bool:
    """
    Проверяет, что новость не слишком старая и не датирована "будущим"
    (что обычно означает баг/рассинхрон времени в ленте).
    Если дату публикации получить не удалось — не отбрасываем новость,
    чтобы не терять материалы без метки времени.
    """
    ts = get_published_ts(entry)
    if ts is None:
        return True
    now_ts = time.time()
    if ts > now_ts + FUTURE_TOLERANCE_MINUTES * 60:
        return False
    if now_ts - ts > MAX_NEWS_AGE_HOURS * 3600:
        return False
    return True


def translate_to_russian(text: str) -> str | None:
    """
    Переводит текст на русский, если он на другом языке.
    Возвращает None, если перевод не удался или язык уже русский
    (чтобы не отправлять дубли для русскоязычных заголовков).
    Делает до 3 попыток с паузой, если сервис перевода временно
    отвечает ошибкой "слишком много запросов".
    """
    if not text:
        return None

    cyrillic_chars = sum(1 for ch in text if "а" <= ch.lower() <= "я")
    if cyrillic_chars > len(text) * 0.3:
        return None

    for attempt in range(1, 4):
        try:
            translated = GoogleTranslator(source="auto", target="ru").translate(text)
            if translated and translated.strip().lower() != text.strip().lower():
                return translated
            return None
        except Exception as exc:
            print(f"Попытка перевода {attempt}/3 не удалась: {exc}")
            if attempt < 3:
                time.sleep(5 * attempt)
    return None


def send_to_telegram(title: str, link: str) -> bool:
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    translation = translate_to_russian(title)
    if translation:
        text = f"{title}\nПеревод: {translation}\n{link}"
    else:
        text = f"{title}\n{link}"

    try:
        resp = requests.post(
            url,
            data={
                "chat_id": CHANNEL_ID,
                "text": text,
                "disable_web_page_preview": False,
            },
            timeout=15,
        )
        if resp.status_code != 200:
            print(f"Ошибка отправки в Telegram: {resp.status_code} {resp.text}")
            return False
        return True
    except requests.RequestException as exc:
        print(f"Сетевая ошибка при отправке в Telegram: {exc}")
        return False


def clean_html(raw: str) -> str:
    return re.sub("<[^<]+?>", "", raw or "").strip()


def check_feeds() -> None:
    seen_data = load_seen()
    seen_links = set(seen_data["links"])
    recent_titles = list(seen_data["titles"])

    # Кандидаты на отправку: (инсайдерский_балл, время_публикации, заголовок, ссылка)
    candidates: list[tuple[int, float, str, str]] = []

    paywall_skipped = 0
    stale_skipped = 0
    duplicates_skipped = 0

    for feed_url in ALL_FEEDS:
        print(f"Проверяю ленту: {feed_url}")
        try:
            feed = feedparser.parse(feed_url)
        except Exception as exc:
            print(f"Не удалось прочитать ленту {feed_url}: {exc}")
            continue

        for entry in feed.entries:
            link = entry.get("link", "")
            title = clean_html(entry.get("title", ""))
            summary = clean_html(entry.get("summary", ""))

            if not link or link in seen_links:
                continue

            # Для Google News ссылки уже отфильтрованы поисковым запросом,
            # но для прямых лент (ТАСС/РИА) нужна собственная фильтрация.
            if not (feed_url in PRE_FILTERED_FEEDS or is_relevant(title, summary)):
                continue

            if is_paywalled(title, summary, link):
                paywall_skipped += 1
                seen_links.add(link)  # не проверяем повторно каждый прогон
                print(f"Пропущено (платный доступ): {title}")
                continue

            if not is_fresh(entry):
                stale_skipped += 1
                seen_links.add(link)  # устаревшей новости свежее не станет
                print(f"Пропущено (не свежая/некорректная дата): {title}")
                continue

            if is_duplicate_title(title, recent_titles):
                duplicates_skipped += 1
                seen_links.add(link)
                print(f"Пропущено как дубль: {title}")
                continue

            published_ts = get_published_ts(entry) or 0.0
            candidates.append((insider_score(title, summary), published_ts, title, link))

    # Сортируем: сначала инсайдерские материалы, затем более свежие по времени.
    candidates.sort(key=lambda c: (c[0], c[1]), reverse=True)
    selected = candidates[:MAX_NEWS_PER_RUN]

    sent = 0
    for _score, _ts, title, link in selected:
        if send_to_telegram(title, link):
            sent += 1
            recent_titles.append(normalize_title(title))
            seen_links.add(link)
            time.sleep(1)  # небольшая пауза, чтобы не спамить API
        # если отправка не удалась — ссылку в seen не добавляем,
        # попробуем отправить её в следующий раз

    save_seen(seen_links, recent_titles)

    if sent == 0:
        print("Подходящих новостей нет — ничего не отправлено, ждём следующего запуска.")
    else:
        print(f"Отправлено новостей: {sent} (из {len(candidates)} подходящих кандидатов)")

    print(
        f"Пропущено: платных — {paywall_skipped}, "
        f"устаревших/с некорректной датой — {stale_skipped}, "
        f"дублей — {duplicates_skipped}"
    )


if __name__ == "__main__":
    check_feeds()
